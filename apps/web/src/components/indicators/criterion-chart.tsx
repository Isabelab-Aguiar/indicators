...
export function CriterionChart({ stat, patients, animationDelay = 0 }: CriterionChartProps) {
  ...
  return (
    <>
      ...
      >
        <AccentBar tone={tone} />
        <BackgroundGlow tone={tone} />
        <div className="relative flex flex-col">
          <CriterionHeader stat={stat} tone={tone} />
          <ProgressSection stat={stat} tone={tone} delay={animationDelay + 0.1} />
          <Divider />
          <DetailRow
            label="Atingiram"
            count={stat.achieved}
...
function AccentBar({ tone }: { tone: CriterionColor }) {
  return <div className={cn('absolute inset-x-0 top-0 h-[2px]', tone.bar)} />
}

function BackgroundGlow({ tone }: { tone: CriterionColor }) {
  return (
    <div
      aria-hidden
...
function CriterionHeader({ stat, tone }: { stat: C3CriterionStat; tone: CriterionColor }) {
  return (
    <div className="flex items-start justify-between gap-3 p-5 pb-4">
      ...
      <div className="text-right">
        <p className={cn('text-3xl font-bold tabular-nums leading-none tracking-tight', tone.text)}>
          {stat.pctAchieved}
...
function ProgressSection({ stat, tone, delay }: ProgressSectionProps) {
  return (
...
function Divider() {
  return <div className="border-border/80 border-t" />
}
...
function DetailRow({ label, count, tone, direction, onClick }: DetailRowProps) {
  const Icon = direction === 'up' ? ArrowUpRight : ArrowDownRight
  const iconColor = direction === 'up' ? tone.text : 'text-muted-foreground'
  const countColor = direction === 'up' ? tone.text : 'text-foreground'
  ...
  return (
    ...
    >
      ...
      <span className="flex items-center gap-2.5">
        <span className={cn('text-sm font-bold tabular-nums leading-none', countColor)}>
          {count}
        </span>
