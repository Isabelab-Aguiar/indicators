...
import { C7PatientDrawer } from './c7-patient-drawer'

type DrawerFilter = 'achieved' | 'not-achieved' | null

interface C7CriterionChartProps {
  stat: C7CriterionStat
  patients: C7PatientRow[]
  animationDelay?: number
}

export function C7CriterionChart({ stat, patients, animationDelay = 0 }: C7CriterionChartProps) {
    ...
    <>
      ...
      >
        ...
        <div className="relative flex flex-col">
          <ChartHeader stat={stat} tone={tone} />
...
function ChartHeader({ stat, tone }: { stat: C7CriterionStat; tone: CriterionColor }) {
  return (
    <div className="flex items-start justify-between gap-3 p-5 pb-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-muted-foreground mt-0.5 text-[11px] font-medium">{stat.ageRange}</p>
          <p className="text-muted-foreground mt-0.5 text-[11px] font-medium tabular-nums">
            {stat.eligible} elegível{stat.eligible !== 1 ? 'eis' : ''}
          </p>
        </div>
      </div>
