...
import { C6PatientDrawer } from './c6-patient-drawer'

type DrawerFilter = 'achieved' | 'not-achieved' | null

interface C6CriterionChartProps {
  stat: C6CriterionStat
  patients: C6PatientRow[]
  animationDelay?: number
}

export function C6CriterionChart({ stat, patients, animationDelay = 0 }: C6CriterionChartProps) {
...
function ChartHeader({ stat, tone }: { stat: C6CriterionStat; tone: CriterionColor }) {
  return (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-muted-foreground mt-0.5 text-[11px] font-medium tabular-nums">
            {stat.total} idoso{stat.total !== 1 ? 's' : ''}
          </p>
        </div>
      </div>
