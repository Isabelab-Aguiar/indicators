...
export function C7DrawerProgress({ pct, color }: { pct: number; color: CriterionColor }) {
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between text-[11px] font-medium">
        <span className="text-muted-foreground">Cobertura do critério (elegíveis)</span>
...
export function C7PatientItem({
  patient,
  achieved,
  onSelect,
}: {
  patient: C7PatientRow
  achieved: boolean
  onSelect: () => void
}) {
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1">
            <p className="text-muted-foreground text-xs">
              {patient.age} anos · Microárea {patient.microarea}
            </p>
            <span className="text-muted-foreground text-xs font-semibold tabular-nums">
              {patient.pctScore.toFixed(0)}%
            </span>
