...
import { Users, TrendingUp, Award, AlertTriangle, Syringe, Baby } from 'lucide-react'
import { cn } from '@repo/ui'
import type { C2Classification, C2CriterionStat, C2PatientRow } from '@repo/types'
import { CLASSIFICATION_STYLES } from './c3-colors'

interface C2SummaryBarProps {
  total: number
  avgScore: number
...
export function C2SummaryBar({
  ...
  const derived = useMemo(() => {
    ...
    const semPrimeiraConsulta = criteriaStats.find((s) => s.id === 'A')
    const semVacinas = criteriaStats.find((s) => s.id === 'E')
    return {
      pending,
      semPrimeiraConsultaPct: semPrimeiraConsulta ? 100 - semPrimeiraConsulta.pctAchieved : 0,
      semVacinasPct: semVacinas ? 100 - semVacinas.pctAchieved : 0,
    }
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
      ...
      <KpiCard
        icon={Users}
        label="Sem 1ª consulta (A)"
        value={
          <div>
            <span className="text-foreground text-2xl font-bold tabular-nums">
              {derived.semPrimeiraConsultaPct.toFixed(1)}
            </span>
            <span className="text-muted-foreground ml-1 text-xs">%</span>
            <MiniProgressBar pct={derived.semPrimeiraConsultaPct} hex="#f59e0b" />
          </div>
        }
        delay={0.2}
        accent="bg-amber-500/10 text-amber-600 dark:text-amber-400"
      ...
      <KpiCard
        icon={Syringe}
        label="Vacinas incompletas (E)"
        value={
          <div>
            <span className="text-foreground text-2xl font-bold tabular-nums">
              {derived.semVacinasPct.toFixed(1)}
            </span>
            <span className="text-muted-foreground ml-1 text-xs">%</span>
            <MiniProgressBar pct={derived.semVacinasPct} hex="#ef4444" />
          </div>
        }
        delay={0.25}
