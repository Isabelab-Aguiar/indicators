...
export function C5EmptyState({ onImport }: { onImport: () => void }) {
  return (
    <div className="border-border flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed py-24">
      <div className="bg-muted flex h-14 w-14 items-center justify-center rounded-2xl">
        <Stethoscope className="text-muted-foreground h-6 w-6" />
      </div>
      <div className="text-center">
        <p className="text-foreground text-sm font-semibold">Nenhum registro C5 encontrado</p>
        <p className="text-muted-foreground mt-1 max-w-xs text-xs leading-relaxed">
          Importe uma planilha CSV com os dados de pessoas com hipertensão para visualizar os
...
export function C5SkeletonGrid() {
  return (
