...
import { buildC6Criteria, buildC6Sections } from './c6-drawer-data'
import { C6PatientItem, C6DrawerProgress } from './c6-drawer-item'
...
export function C6PatientDrawer({
  ...
  const [selected, setSelected] = useState<C6PatientRow | null>(null)
    ...
    <>
      <AnimatePresence>
        ...
        >
          ...
          <div className="border-border space-y-3 border-b px-5 py-3">
            <C6DrawerProgress pct={stat.pctAchieved} color={color} />
          ...
          <div className="flex-1 overflow-y-auto">
            {filtered.length === 0 ? (
              ...
              filtered.map((p, i) => (
                ...
                >
                  <C6PatientItem patient={p} achieved={achieved} onSelect={() => setSelected(p)} />
                </motion.div>
              ))
            )}
          </div>
      ...
      {selected && (
        <PatientDetailModal
          ...
          criteria={buildC6Criteria(selected)}
          sections={buildC6Sections(selected)}
