...
import { C4ImportacaoSection } from '@/features/c4/components/C4ImportacaoSection'
import { C4ExportarCsv } from '@/features/c4/components/C4ExportarCsv'
import { C4Dashboard } from './c4-dashboard'
...
export function C4Tabs() {
  const [activeTab, setActiveTab] = useState<TabKey>('analise')
  const { breakdown } = useC4Analytics()
    ...
    >
      <div className="border-border flex flex-wrap items-center justify-between gap-2 border-b pb-3">
        ...
        <IndicatorFilters
          ...
          activeTab={activeTab}
          exportSlot={activeTab === 'analise' ? <C4ExportarCsv breakdown={breakdown} /> : undefined}
        />
      </div>

      <Tabs.Content value="analise" asChild>
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
          <C4Dashboard onImport={() => setActiveTab('importar')} />
        </motion.div>
      </Tabs.Content>
      ...
      <Tabs.Content value="simulador" asChild>
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
          <IndicatorSimulator indicator={INDICATORS.c4} />
        </motion.div>
      </Tabs.Content>
    </Tabs.Root>
  )
