...
import { C5PatientDrawer } from './c5-patient-drawer'

type DrawerFilter = 'achieved' | 'not-achieved' | null

interface C5CriterionChartProps {
  stat: C5CriterionStat
  patients: C5PatientRow[]
  animationDelay?: number
}

export function C5CriterionChart({ stat, patients, animationDelay = 0 }: C5CriterionChartProps) {
...
function ChartHeader({ stat, tone }: { stat: C5CriterionStat; tone: CriterionColor }) {
  return (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-muted-foreground mt-0.5 text-[11px] font-medium tabular-nums">
            {stat.total} paciente{stat.total !== 1 ? 's' : ''}
          </p>
        </div>
      </div>
