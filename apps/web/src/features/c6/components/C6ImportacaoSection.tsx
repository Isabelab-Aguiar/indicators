...
export function C6ImportacaoSection() {
  ...
  const importMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      return apiClient.post(`/c6/import?periodo=${buildPeriodo()}`, form, {
        headers: { 'Content-Type': undefined },
      })
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.c6.all(esfId) })
      setSelectedFile(null)
      toast({ title: 'Arquivo enviado! Dados C6 importados com sucesso.' })
    },
  ...
  function handleDownloadTemplate() {
    window.open('/api/v1/c6/import/template', '_blank')
  }
