...
export function C5ImportacaoSection() {
  ...
  const importMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      return apiClient.post(`/c5/import?periodo=${buildPeriodo()}`, form, {
        headers: { 'Content-Type': undefined },
      })
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.c5.all(esfId) })
      setSelectedFile(null)
      toast({ title: 'Arquivo enviado! Dados C5 importados com sucesso.' })
    },
  ...
  function handleDownloadTemplate() {
    window.open('/api/v1/c5/import/template', '_blank')
  }
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <CardDescription className="mt-1 text-xs">
              CSV com colunas: Nome, A, B, C, D. Valores aceitos: Sim/Não, 1/0, true/false.
            </CardDescription>
          </div>
          <Button
            variant="ghost"
