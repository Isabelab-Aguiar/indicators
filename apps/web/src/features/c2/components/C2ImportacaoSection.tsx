...
function buildPeriodo(): string {
  const now = new Date()
  return `${now.getFullYear()}-Q${getQuadrimestre(now)}`
}

export function C2ImportacaoSection() {
  ...
  const importMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      return apiClient.post(`/c2/import?periodo=${buildPeriodo()}`, form, {
        headers: { 'Content-Type': undefined },
      })
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.c2.all(esfId) })
      setSelectedFile(null)
      toast({ title: 'Dados C2 importados com sucesso.' })
    },
    onError: () => toast({ title: 'Falha ao importar arquivo', variant: 'destructive' }),
  })
