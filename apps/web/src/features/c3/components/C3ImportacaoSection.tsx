...
export function C3ImportacaoSection() {
  ...
  const importMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      return apiClient.post(endpoint, form, { headers: { 'Content-Type': undefined } })
    },
    onSuccess: () => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm font-semibold">Importar dados de Pré-Natal</CardTitle>
        <CardDescription className="text-xs">
          CSV ou PDF exportado do e-SUS com dados de gestantes. Os registros serão vinculados à sua
          ESF.
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        >
          ...
          <p className="text-muted-foreground mt-1 text-xs">CSV ou PDF do e-SUS · máx. 10MB</p>
        </div>
